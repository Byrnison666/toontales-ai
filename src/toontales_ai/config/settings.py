...
class Settings(BaseSettings):
    ...
    # Стартовый бонус кредитов новому пользователю при регистрации (~хватает на 1
    # ролик = estimate_run_cost(6 сцен) ≈ 1680). Нужен, чтобы юзер-фронт не был
    # мёртвым для новичка (генерация требует полный max_budget на балансе).
    # ИЗВЕСТНЫЙ РИСК: бесплатные кредиты абузятся регистрацией множества аккаунтов —
    # приемлемо для MVP/демо; в проде нужен email-verify/captcha или бонус=0.
