...
def _system_prompt(scene_count: int, clip_seconds: int) -> str:
    ...
    return (
        ...
        f"(классическая 2D-мультипликация), БЕЗ фотореализма. НЕ упоминай в "
        f"image_prompt названия студий, брендов или франшиз (Disney, Pixar, Marvel "
        f"и т.п.) — только описание стиля, иначе модерация генератора отклонит кадр. "
        f"Каждый image_prompt описывай как рисованную мультипликацию в этом стиле: {style}"
    )
