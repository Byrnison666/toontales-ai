...
def _system_prompt(scene_count: int, clip_seconds: int) -> str:
    ...
    return (
        ...
        f"ВАЖНО про стиль: все кадры — в едином мультяшном рисованном стиле "
        f"(классическая 2D-мультипликация), БЕЗ фотореализма. Придумывай ОРИГИНАЛЬНЫХ "
        f"персонажей и образы. НЕ упоминай названия студий/брендов/франшиз (Disney, "
        f"Pixar, Marvel и т.п.) И не описывай персонажей/объекты, визуально повторяющих "
        f"известных защищённых персонажей (напр. говорящая жёлтая губка под водой ≈ "
        f"SpongeBob, знаменитые маскоты, супергерои, узнаваемые мультгерои) — иначе "
        f"модерация генератора отклонит кадр (и по тексту, и по картинке). Каждый "
        f"image_prompt описывай как рисованную мультипликацию в этом стиле: {style}"
    )
