...
def upgrade() -> None:
    # Блокируем users и tasks на запись до проверки drain и пересчёта: API-процесс
    # во время миграции жив и мог бы между count(*) и пересчётом создать новый run
    # (холд в СТАРОЙ шкале) или изменить баланс. SHARE ROW EXCLUSIVE конфликтует с
    # ROW EXCLUSIVE (INSERT/UPDATE), но пропускает чтения; сама миграция пишет
    # свободно. Снимается автоматически на COMMIT транзакции миграции.
    op.execute("LOCK TABLE users, tasks IN SHARE ROW EXCLUSIVE MODE")
    ...
    # Идемпотентность: для пользователей с балансом > 0 повторный прогон no-op —
    # маркер уже есть, ON CONFLICT глушит вставку, UPDATE трогает только строки из
    # RETURNING. Не закрыт лишь искусственный случай: баланс был ровно 0 при
    # миграции (маркера нет), потом пополнен, потом РУЧНОЙ повторный прогон уже
    # применённой миграции — нормальный повтор блокирует alembic_version, ручной
    # реран raw-SQL неподдерживаем. Принятое ограничение.
